...
├── fit/
│   ├── parameters.py       FitParameter, ParameterMap (free/fixed bookkeeping)
│   ├── result.py           FitResult
...
```python
from scipas.analysis.lifetime import generate_analytical_lt_spectrum, generate_random_lt_spectrum
...
```python
from scipas.analysis.lifetime import LifetimeFitter, FitParameter
...
Internally the module separates concerns: `parameters.py` holds
`FitParameter` and `ParameterMap` — the bookkeeping that translates between
named free/fixed parameters and the flat optimizer vector (including the
dependent-intensity constraint); `fitter.py` holds the physics (forward
model) and the optimization; `result.py` the result container.
...
```python
from scipas.analysis.lifetime.inversion.utils import t0_scan
